...
import com.asiainfo.utils.aelstad.keccakj.io.BitInputStream;
...
public class KeccackSponge {
		...
		public long transformBits(byte[] input, long inputOff, byte[] output, long outputOff, long bitLen) {
			...
			while(bitLen > 0) {
				...
				if((ratePos & 7)==0 && (inputOff&7)==0 && (outputOff&7)==0 && (chunk&7)==0) {
					keccack1600.bytesOp(KeccackStateUtils.StateOp.XOR_TRANSFORM, ratePos>>3, output, (int) (outputOff>>3), input, (int) (inputOff>>3), chunk>>3);
				} else {
					keccack1600.bitsOp(KeccackStateUtils.StateOp.XOR_TRANSFORM, ratePos, output, outputOff, input, inputOff, chunk);
				}
	...
	public FilterOutputStream getTransformingSqueezeStream(final java.io.OutputStream target) {
